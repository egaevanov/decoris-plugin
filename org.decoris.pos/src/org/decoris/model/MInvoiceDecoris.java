...
public class MInvoiceDecoris extends MInvoice implements DocAction
{

	/**
	 * 
	 */
	private static final long serialVersionUID = 1543160464310054801L;


	public MInvoiceDecoris(MInOut ship, Timestamp invoiceDate) {
		super(ship, invoiceDate);
		// TODO Auto-generated constructor stub
	}
	public MInvoiceDecoris(MInvoiceBatch batch, MInvoiceBatchLine line) {
		super(batch, line);
		// TODO Auto-generated constructor stub
	}

	public MInvoiceDecoris(MOrder order, int C_DocTypeTarget_ID,
	...
	public MInvoiceDecoris(Properties ctx, int C_Invoice_ID, String trxName) {
		super(ctx, C_Invoice_ID, trxName);
		// TODO Auto-generated constructor stub
	}
	public MInvoiceDecoris(Properties ctx, ResultSet rs, String trxName) {
	...
	public String completeIt()
	{
		...
		if(!isReversal()){
			...
			MInvoice autogenerate = new MInvoice(getCtx(), getC_Invoice_ID(), get_TrxName());
			invoiceVendorBeforeComplete(autogenerate);
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!isSOTrx()
				...
				&& !isReversal() && !get_ValueAsBoolean("IsPrepayment"))
			{
		...
		//  Added If, SS, Surya Sentosa, Kosta Consulting, exclude open balance when invoice proforma
		if (!get_ValueAsBoolean("IsPrepayment")){
...
//		counter.setBPartner(counterBP);// was set on copyFrom
	...
	public void invoiceVendorBeforeComplete (MInvoice inv){
		...
		if(C_Decoris_PostingMethod_ID > 0){
			...
			isCompletePay = post.isCompleteDocStatusPay();		
			
			if(inv.getC_Order_ID() > 0){
				ordPost = new MOrder(inv.getCtx(), inv.getC_Order_ID(), inv.get_TrxName());
			}
			...
			payment.setC_Currency_ID(ordPost.getC_Currency_ID());
			payment.setTenderType(ordPost.getPaymentRule());
