...
import org.decoris.model.MDecorisPOSLine;
...
public class DecorisProcessPOS extends SvrProcess{
	...
	MDecorisPOS DecPos = null;
	X_C_DecorisTemp DecPos_Temp = null;
	MOrder Order = null;
	...
	protected void prepare() {
		...
		for (int i = 0 ; i < para.length ;i++){
			...
			else if(name.equals("C_DecorisTemp_ID"))
	...
	protected String doIt() throws Exception {
		...
		try{
			...
			DecPos_Temp = new X_C_DecorisTemp(getCtx(), p_C_DecorisTemp_ID, get_TrxName());
			
			DecPos = new MDecorisPOS(getCtx(), 0, get_TrxName());
			...
			try {
				...
				while (rs.next()) {
					X_C_DecorisLineTemp DecPosLine_Temp = new X_C_DecorisLineTemp(getCtx(), rs.getInt(1), get_TrxName());
					MDecorisPOSLine DecPOSLine = new MDecorisPOSLine(getCtx(), 0, get_TrxName());					
			...
			MDecorisPOS Dec = new MDecorisPOS(getCtx(), DecPos.getC_DecorisPOS_ID(),get_TrxName());
...
//								+ Env.getAD_Client_ID(getCtx());
		...
		if ( ok ){
			...
			DecPos_Temp.setC_DecorisPOS_ID(DecPos.getC_DecorisPOS_ID());
			DecPos_Temp.save();
