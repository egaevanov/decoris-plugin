...
public class PosCloseCash {
	...
	protected Vector<Vector<Object>> getCloseCashData(int AD_Client_ID,int AD_Org_ID, Timestamp DateTrx1,Timestamp DateTrx2,int CreatedBYPOS_ID,String KriteriaData,IMiniTable MiniTable) {
		...
		SQLGetCloceCashData.append(" FROM T_CloseCashSummary ");
		...
		try {
			...
			Vector<Object> line = new Vector<Object>(10);
				...
				line.add(rsClsd.getBigDecimal(5));
	...
	protected Vector<Vector<Object>> getCloseCashSummary(int AD_Client_ID,int AD_Org_ID, Timestamp DateTrx,Timestamp CloseCashDate,int CreatedByPOS_ID,String documentNo,int seqTutupKas ) {
	
		StringBuilder SQLSummary = new StringBuilder();
		SQLSummary.append(" SELECT Cash,BankPayment,LeasingPayment,LainLain,OtherLeasingPayment ");
		SQLSummary.append(" FROM T_CloseCashSummary ");
