 ...
 *
 */
public class WPosPenjualanRollBack extends PosPenjualanRollBack implements
	...
	private boolean dyinit() {
		...
		MLookup lookupNotaOld = MLookupFactory.get(ctx, form.getWindowNo(), 0,
				1000934, DisplayType.Search);
		noNotaOld = new WSearchEditor("C_DecorisPOS_ID", true, false, true,
				lookupNotaOld);
		...
		try {
			...
			if (name.equals("C_BPartner_ID")) {
										...
										public void onEvent(Event event)
											...
											if (event.getTarget() == panel
												...
												ListModelTable modelP = new ListModelTable(
														data);
												modelP.addTableModelListener(WPosPenjualanRollBack.this);
				...
				if (C_Decoris_PreSales_ID > 0) {
					...
					X_C_Decoris_PreSales preSales = new X_C_Decoris_PreSales(
							ctx, C_Decoris_PreSales_ID, null);

					if (preSales.getStatus().equals(
							X_C_Decoris_PreSales.STATUS_Closed)
							|| preSales.getStatus().equals(
									X_C_Decoris_PreSales.STATUS_Completed)) {
						FDialog.info(
								windowNo,
								null,
								"",
								"Tidak dapat diprocess,Status Presales yang anda inputkan sudah Complete atau Closed",
								"Info");
						return;
					}
					...
					if (preSales.getM_PriceList_ID() > 0) {
						MPriceList PL = new MPriceList(ctx,
								preSales.getM_PriceList_ID(), null);
						ArrayList<KeyNamePair> listPL = new ArrayList<KeyNamePair>();
						listPL.add(new KeyNamePair(PL.getM_PriceList_ID(), PL
								.getName()));
						DaftarHargaSearch.removeAllItems();
					...
					if (preSales.getM_Warehouse_ID() > 0) {
						MWarehouse WH = new MWarehouse(ctx,
								preSales.getM_Warehouse_ID(), null);
						...
						ArrayList<KeyNamePair> listLoc = loadLocator(
								preSales.getM_Warehouse_ID(),
								preSales.getAD_Org_ID());
						Lokasi.removeAllItems();
						for (KeyNamePair priceList : listLoc)
							Lokasi.appendItem(priceList.getName());
					...
					paymentLeasing.setValue(preSales
							.getJumlahPembayaranLeasing());
					paymentHutang
							.setValue(preSales.getJumlahPembayaranHutang());
					...
					BigDecimal totalBayarAll = tunai.add(bank).add(leasing)
							.add(hutang);

					totalBayarTextBox.setText(format.format(preSales
							.getTotalBayar()));

					BigDecimal kembali = totalBayar.subtract(preSales
							.getTotalBelanja());
					kembaliTextBox.setText(format.format(kembali));
					diskonTextBox.setText(format.format(preSales
							.getTotalDiskon()));
					totalBelanjaTextBox.setText(format.format(preSales
							.getTotalBelanja()));
					TotalHeaderTextBox.setText(format.format(preSales
							.getTotalBelanja()));
					totalBayarTunaiTextBox.setText(format.format(preSales
							.getTotalBayar()));
					totalBayarTextBox.setText(format.format(totalBayarAll));
					kembalianTextBox.setText(format.format(preSales
							.get_Value("TotalKembalian")));
					kembaliTextBox.setText(format.format(preSales
							.get_Value("TotalKembalian")));
					DPPTextBox.setText(format.format(preSales.getdpp()));
					PPNTextBox.setText(format.format(preSales.getTaxAmt()));
					totalBrutoTextBox
							.setText(format.format(preSales.gettotal()));
				...
				StringBuilder SQLGetPreOrder = new StringBuilder();
				SQLGetPreOrder.append("SELECT C_Decoris_PreOrder_ID ");
				SQLGetPreOrder.append("FROM C_DecorisPOS ");
				SQLGetPreOrder.append("WHERE AD_Client_ID = ? ");
				SQLGetPreOrder.append("AND C_Decoris_PreOrder_ID = ? ");
				int C_Decoris_PreOrder_ID = ((Integer) value).intValue();

				if (C_Decoris_PreOrder_ID > 0) {

					X_C_Decoris_PreOrder preOrder = new X_C_Decoris_PreOrder(
							ctx, C_Decoris_PreOrder_ID, null);
					Integer C_Decoris_PreOrderCek_ID = DB.getSQLValueEx(null,
							SQLGetPreOrder.toString(), new Object[] {
									AD_Client_ID, C_Decoris_PreOrder_ID });

					if (C_Decoris_PreOrderCek_ID == null
							|| C_Decoris_PreOrderCek_ID < 0) {

						C_Decoris_PreOrderCek_ID = 0;

					}

					if (C_Decoris_PreOrderCek_ID > 0) {
						FDialog.info(
								windowNo,
								null,
								"",
								"Preorder Yang Terinput Sudah Pernah Terproses",
								"Peringatan");
						return;
					}
					...
					if (!isPass) {
						FDialog.info(
								...
								null,
								"",
								"Tidak dapat diproses,Status PreOrder yang anda inputkan Belum di Approve",
								"Info");
						return;
					}
					...
					if (preOrder.getM_PriceList_ID() > 0) {
						MPriceList PL = new MPriceList(ctx,
								preOrder.getM_PriceList_ID(), null);
						ArrayList<KeyNamePair> listPL = new ArrayList<KeyNamePair>();
						listPL.add(new KeyNamePair(PL.getM_PriceList_ID(), PL
								.getName()));
						DaftarHargaSearch.removeAllItems();
					...
					if (preOrder.get_ValueAsInt("M_Warehouse_ID") > 0) {
						MWarehouse WH = new MWarehouse(ctx,
								preOrder.get_ValueAsInt("M_Warehouse_ID"), null);
						ArrayList<KeyNamePair> listWH = new ArrayList<KeyNamePair>();
						listWH.add(new KeyNamePair(WH.getM_Warehouse_ID(), WH
								.getName()));
						GudangSearch.removeAllItems();
					...
					salesSearch
							.setValue(preOrder.get_ValueAsInt("SalesRep_ID"));
					salesSearch.setReadWrite(false);
					...
					paymentLeasing.setValue(preOrder.gettotprodprice()
							.subtract(preOrder.getdpamt()));
					leasingProviderSearch.setValue("Spektra");
					keteranganTextBox.setValue("Cicilan = "
							+ preOrder.getangsuran() + " " + "\n" + "Tenor = "
							+ preOrder.gettop() + "X");

					totalBelanjaTextBox.setText(format.format(preOrder
							.gettotprodprice()));
					TotalHeaderTextBox.setText(format.format(preOrder
							.gettotprodprice()));
					totalBrutoTextBox.setText(format.format(preOrder
							.gettotprodprice()));
				...
				MDecorisPOS decp = new MDecorisPOS(ctx, C_DecorisPOS_ID, null);
	...
	private void process(boolean IsOtorisasiHutang, boolean IsOtorisasiPriceLimit,int Spv_ID,
		...
		if (!isForceLimitPrice) {
			...
			for (int i = 0; i < salesTable.getRowCount(); i++) {
				...
				BigDecimal priceLimit = DB.getSQLValueBDEx(null,
						sqlProductPrice, new Object[] { AD_Client_ID,
								M_PriceList_Version_ID, prod.getKey() });
				...
				if (price.compareTo(priceLimit) < 0) {
					FDialog.warn(
							...
							null,
							"",
							"Harga Yang Anda Inputkan Atas Produk "
									+ prod.getName()
									+ " Lebih Rendah Dari Limit Harga",
							"Peringatan");
					return;
				}
		...
		Integer M_WareHouse_ID = getIDFromComboBox(GudangSearch,
				MWarehouse.Table_Name, MWarehouse.COLUMNNAME_Name);
		...
		int decDoc_ID = DB.getSQLValueEx(null, decPOsdocType, new Object[] {
				AD_Client_ID, docDec });
		decPos_temp.setC_DocType_ID(decDoc_ID);
		decPos_temp.setClientOrg(Env.getAD_Client_ID(ctx),
				(int) orgSearch.getValue());
		...
		int C_BPartner_Location_ID = DB.getSQLValueEx(ctx.toString(),
				sqlBPLoc.toString(), new Object[] { C_BPartner_ID });
		...
		decPos_temp.setDescription((String) keteranganTextBox.getValue()
				.toString());
		decPos_temp.setM_PriceList_ID(M_Pricelist_ID);
		decPos_temp.setDiscountAmt(totalDisc);
		decPos_temp.setM_Warehouse_ID((int) getIDFromComboBox(GudangSearch,
				MWarehouse.Table_Name, MWarehouse.COLUMNNAME_Name));
		...
		for (int i = 0; i < miniTable.getRowCount(); i++) {
			...
			X_C_DecorisLineTemp decPOSLine_temp = new X_C_DecorisLineTemp(ctx,
			...
			int ii = DB.getSQLValue(decPos_temp.get_TrxName(), sqlLine,
					decPos_temp.getC_DecorisTemp_ID());

			// DecorisLine
			decPOSLine_temp.setClientOrg(decPos_temp);
			decPOSLine_temp.setC_DecorisTemp_ID(decPos_temp
					.getC_DecorisTemp_ID());
			...
			decPOSLine_temp.setPriceList((BigDecimal) miniTable
					.getValueAt(i, 3));
			decPOSLine_temp.setPriceEntered((BigDecimal) miniTable.getValueAt(
					i, 4));
			decPOSLine_temp.setQtyOrdered((BigDecimal) miniTable.getValueAt(i,
					2));
		...
		String process_id = DB.getSQLValueStringEx(null,
				SQLGetProcess.toString());
		...
		list.add(new ProcessInfoParameter("AD_Client_ID", decPos_temp
				.getAD_Client_ID(), null, null, null));
		list.add(new ProcessInfoParameter("AD_Org_ID", decPos_temp
				.getAD_Org_ID(), null, null, null));
		list.add(new ProcessInfoParameter("C_DecorisTemp_ID", decPos_temp
				.getC_DecorisTemp_ID(), null, null, null));
		list.add(new ProcessInfoParameter("DocAction", m_docAction, null, null,
				null));
		...
		boolean ada = new DecorisProcessPOS().startProcess(Env.getCtx(), pi,
				Trx.get(trxName, true));

		X_C_DecorisTemp decPos_temp2nd = new X_C_DecorisTemp(
				decPos_temp.getCtx(), decPos_temp.getC_DecorisTemp_ID(), null);
		...
		if (C_OrderResult_ID <= 0) {
			...
			FDialog.info(
					...
					null,
					"",
					"Process Transaksi Gagal, Silakan Cek Kelengkapan Data Atau Hubungi Admin",
					"Info Process POS");
			return;
			...
			if (M_InOut_ID > 0) {
				...
				if (DocStatus.equals(MInOut.DOCSTATUS_Drafted)) {
					...
					for (int i = 0; i < miniTable.getRowCount(); i++) {
						...
						KeyNamePair product = (KeyNamePair) miniTable
								.getValueAt(i, 1);
						KeyNamePair POSLoc = (KeyNamePair) miniTable
								.getValueAt(i, 7);
						int POSLineNo = (i + 1) * 10;

						int POSProd_ID = product.getKey();
						int POSLoc_ID = POSLoc.getKey();
						BigDecimal POSqQty = ((BigDecimal) miniTable
								.getValueAt(i, 2)).setScale(2);
		...
		MDecorisPOS decPos = new MDecorisPOS(decPos_temp2nd.getCtx(),
		...
		if (C_Decoris_PreSales_ID > 0) {
			...
			X_C_Decoris_PreSales pSales = new X_C_Decoris_PreSales(ctx,
					C_Decoris_PreSales_ID, null);
						...
						new EventListener<Event>() {
							...
							public void onEvent(Event event) throws Exception {
								if (event.getTarget() == panel
									...
									String noTlp = "";
									boolean isBedaAlamat = bedaAlamatCheck
											.isChecked();
	...
	private void ClearTableLine(int AD_Client_ID,
			X_C_DecorisTemp X_C_DecorisTemp) {

		StringBuilder SQLDelete = new StringBuilder();
		SQLDelete.append("SELECT C_DecorisLineTemp_ID ");
		SQLDelete.append(" FROM C_DecorisLineTemp ");
		SQLDelete.append(" WHERE AD_Client_ID = ? ");
		SQLDelete.append(" AND C_DecorisTemp_ID = ? ");
		...
		try {
			...
			pstmt.setInt(1, AD_Client_ID);
			pstmt.setInt(2, X_C_DecorisTemp.getC_DecorisTemp_ID());
			rs = pstmt.executeQuery();
			while (rs.next()) {

				X_C_DecorisLineTemp line = new X_C_DecorisLineTemp(ctx,
						rs.getInt(1), X_C_DecorisTemp.get_TrxName());
				line.delete(true, X_C_DecorisTemp.get_TrxName());
			}
		...
		X_C_DecorisTemp.delete(true, X_C_DecorisTemp.get_TrxName());
	...
	private void ValidationPriceLimit(final boolean IsOtorisasiHutang,final Integer SuperVisor_ID, final Timestamp DateApprove,final Integer C_PayTerm_ID ){
		...
		for(int i = 0 ; i<salesTable.getRowCount() ;i++){
			...
			int M_PriceList_Version_ID = DB.getSQLValueEx(null, sql, new Object[] {
					AD_Client_ID, pL_ID, date });

			pp.setM_PriceList_Version_ID(M_PriceList_Version_ID);
			pp.setPriceDate(date);
			
			BigDecimal priceEnt = (BigDecimal) salesTable.getValueAt(i, 4);
			BigDecimal priceLmt = pp.getPriceLimit();
