 ...
 *
 */
public class WPosPenjualan extends PosPenjualan implements IFormController,
	...
	private void init() {
		...
		row.appendCellChild(salesSearch.getComponent(), 2);
		salesSearch.getComponent().setHflex("true");

		row = rows.newRow();
		row.appendCellChild(LokasiLabel.rightAlign(), 1);
		row.appendCellChild(Lokasi, 2);
		...
		row2 = rows2.newRow();
		row2.appendCellChild(DaftarHargaLabel.rightAlign(), 1);
		row2.appendCellChild(DaftarHargaSearch, 2);
		...
		row3 = rows3.newRow();
		row3.appendCellChild(PelangganLabel.rightAlign(), 1);
		Hbox pelangganHbox = new Hbox();
		row3.appendCellChild(pelangganHbox, 2);
		...
		row3 = rows3.newRow();
		row3.appendCellChild(GudangLabel.rightAlign(), 1);
		row3.appendCellChild(GudangSearch, 2);
	...
	protected void configureMiniTableExistingPresales(IMiniTable miniTable) {
		...
		miniTable.setColumnClass(3, BigDecimal.class, true); 		// 4-priceList
		miniTable.setColumnClass(4, BigDecimal.class, true);		// 5-price
		miniTable.setColumnClass(5, String.class, true); 			// 6-diskon
		miniTable.setColumnClass(6, BigDecimal.class, true); 		// 7-totalprice
		miniTable.setColumnClass(7, String.class, true); 			// 8-locator
		miniTable.setColumnClass(8, String.class, true); 			// 9-imei
			...
			if (name.equals("C_BPartner_ID")) {
				...
				if (C_Decoris_PreSales_ID > 0) {
					...
					if(preSales.getStatus().equals(X_C_Decoris_PreSales.STATUS_Closed)
							||preSales.getStatus().equals(X_C_Decoris_PreSales.STATUS_Completed)){
			...
			if (event.equals(Events.ON_FOCUS)) {
				...
				if (AD_Language.toUpperCase().equals("IN_ID")) {
					if (e.getTarget().equals(paymentTunai)) {
						String sTunai = paymentTunai.getValue().toString()
						...
						String sBank = paymentBank.getValue().toString()
								.replaceAll("\\.", "").replaceAll(",", ".");
						...
						String sLeasing = paymentLeasing.getValue().toString()
								.replaceAll("\\.", "").replaceAll(",", ".");
						...
						String sHutang = paymentHutang.getValue().toString()
								.replaceAll("\\.", "").replaceAll(",", ".");
	...
	public void calculate() {
		...
		totalBrutoTextBox.setText(format.format(totalBeforeDiscount));
	...
	private String cekAkses() {
		...
		if (CreatedByPOS_ID <= 0) {
			...
			FDialog.info(windowNo, null, "",
					"User Tidak Mempunyai Akses ke POS", "Info");
			Msg = "error";
		}
	...
	private void addBP() {
		...
		// City-7048
		MLookup lookupCity = MLookupFactory.get(ctx, form.getWindowNo(), 0,
				7048, DisplayType.TableDir);

		final WTableDirEditor cityList = new WTableDirEditor("C_City_ID", true,
				false, true, lookupCity);
		...
		BigDecimal creditLimit = DB.getSQLValueBDEx(null, getCreditLimit,
				new Object[] { AD_Client_ID, CreatedByPOS_ID });
		...
		panel.addActionListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
					...
					// String p_SearchKey =
					// Client.getValue()+"/"+"POS"+System.currentTimeMillis();
	...
	private void process() {
		...
		if (leasingProviderSearch.getValue() != null) {
			String leaseProvid = "";
			leaseProvid = cekBankAccount(leasingProviderSearch.getValue()
					.toString());
			if (leaseProvid != "" || !leaseProvid.isEmpty()) {
				FDialog.warn(form.getWindowNo(), null, leaseProvid, "",
						"Peringatan");
				return;
			}
		}
		...
		if (!isMultiLoc) {
			...
			// String sqlLoca =
			// "SELECT M_Locator_ID FROM M_Locator WHERE AD_Client_ID = ? AND value = '"+
			// loca + "'";
		...
		decPos.setAD_Org_ID((int) orgSearch.getValue());
		decPos.setC_BPartner_ID((int) PelangganSearch.getValue());
...
//		Timestamp dateOrd = Timestamp.valueOf(tglTostr);
		...
		decPos.setDateOrdered(dateOrd);
		decPos.setDescription((String) keteranganTextBox.getValue().toString());
		decPos.setM_PriceList_ID(M_Pricelist_ID);
		decPos.setDiscountAmt(totalDisc);
		decPos.setM_Warehouse_ID((int) getIDFromComboBox(GudangSearch,
				MWarehouse.Table_Name, MWarehouse.COLUMNNAME_Name));
		...
		if (payment3.compareTo(Env.ZERO) > 0) {
			decPos.setPayType3(tipeBayar3.toUpperCase());
		...
		if (payment4.compareTo(Env.ZERO) > 0) {
			decPos.setPayType4(tipeBayar4.toUpperCase());
		}
		...
		if (payment1.compareTo(Env.ZERO) > 0) {
			...
			decPos.setPembayaran1(payment1.subtract(kembalian));
			decPos.set_CustomColumn("TotalKembalian", kembalian);
			decPos.set_CustomColumn("TotalTunai", paymentTunai.getValue());
		}
		if (payment2.compareTo(Env.ZERO) > 0) {
			decPos.setPembayaran2(payment2);
		}
		if (payment3.compareTo(Env.ZERO) > 0) {
			decPos.setPembayaran3(payment3);
		}

		if (payment4.compareTo(Env.ZERO) > 0) {
			decPos.setPembayaran4(payment4);
		}
		...
		for (int i = 0; i < miniTable.getRowCount(); i++) {
			...
			int ii = DB.getSQLValue(decPos.get_TrxName(), sqlLine,
					decPos.getC_DecorisPOS_ID());
			...
			if (isMultiLoc && prod.getProductType().toUpperCase().equals("I")) {
				...
				// loc.getID() + "'";
				// M_Locator_ID_Multi =
				// DB.getSQLValueEx(Env.getCtx().toString(), sqlLoca,
				// AD_Client_ID);
		...
		MDecorisPOS dec = new MDecorisPOS(ctx, decPos.getC_DecorisPOS_ID(),
				null);
		...
		if (payment1.compareTo(Env.ZERO) > 0) {
			...
			mapPospay.put(tipeBayar1, payment1.subtract(kembalian));
		...
		if (payment4.compareTo(Env.ZERO) > 0) {
			...
			mapPospay.put(tipeBayar4, payment4);
			...
			if (tipebayar.equals("TUNAI") || tipebayar.equals("LEASING")
				...
				if (tipebayar.equals("TUNAI") || tipebayar.equals("BANK")) {
					...
					if (tipebayar.equals("TUNAI")) {
						...
						C_BankAcct_ID = DB.getSQLValueEx(ctx.toString(),
								sqlBank.toString(), new Object[] {
										CreatedByPOS_ID, AD_Client_ID });

					} else if (tipebayar.equals("BANK")) {
						Integer b_acct_id = (Integer) bankAccountSearch
								.getValue();
						if (b_acct_id == null) {
							b_acct_id = 0;
						}
				...
				String sqlTender = "SELECT C_POSTenderType_ID FROM C_POSTenderType WHERE name = '"
						+ whereClause + "'";
				int C_POSTenderType_ID = DB.getSQLValueEx(ctx.toString(),
						sqlTender.toString());
				createPOSPayment(ord.getAD_Org_ID(), ord.getC_Order_ID(),
						C_POSTenderType_ID, tenderName, payAmt, leaseProv,
						C_BankAcct_ID);
			...
			if (m_docAction != null && m_docAction.length() > 0) {
				...
				if (!m_docAction.equals("PR")) {
					if (!ord.processIt(m_docAction)) {
						log.warning("Order Process Failed: " + ord + " - "
								+ ord.getProcessMsg());
						throw new IllegalStateException(
								"Invoice Process Failed: " + ord + " - "
										+ ord.getProcessMsg());
					}
				}
			}
		...
		int C_Invoice_ID = checkInvoiceRelated(ord.getC_Order_ID());
						...
						new EventListener<Event>() {
							...
							public void onEvent(Event event) throws Exception {
								if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
									...
									// FDialog.info(windowNo, null, "",
									// "Penjualan Berhasil Di Proses", "Info");
...
//	}
	...
	private void imei() {
		...
		if (imeiIndex != null) {
			...
			Vector<Vector<Object>> data = getProductData(M_ProductIMEI_ID,
					M_Pricelist_ID, bpartnerId, M_LocatorPOS_ID,
					M_AttributeSetInstance_ID, salesTable);
			Vector<String> columnNames = getOISColumnNames();
