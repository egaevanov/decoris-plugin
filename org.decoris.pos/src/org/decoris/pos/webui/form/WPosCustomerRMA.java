 ...
 * @author Tegar N
 *
 * Decoris Refund Penjualan
 *
 */
public class WPosCustomerRMA extends PosCustomerRMA implements IFormController,
EventListener<Event>, WTableModelListener, ValueChangeListener{
	...
	public void valueChange(ValueChangeEvent evt) {
		...
		try {
			...
			if(name.equals("C_DecorisPOS_ID")){
				
				Integer C_DecorisPOS_ID = Integer.valueOf(value.toString());	
				X_C_DecorisPOS pos= new X_C_DecorisPOS(ctx, C_DecorisPOS_ID , null);			
...
//		locatorSearch.setMandatory(true);
		...
		noNota  = new WSearchEditor("C_DecorisPOS_ID",true, false, true,lookupNota);
	...
	protected void configureMiniTable(IMiniTable miniTable) {
		...
		miniTable.setColumnClass(8, BigDecimal.class, false); 	// 8-qty return
	...
	public void process(int supervisor_id){
		...
		if(!DocStatus.equals(MInOut.DOCSTATUS_Completed)){
			...
			FDialog.info(windowNo, null, "", "Return tidak dapat diproses, belum ada pengiriman barang", "Info");
			return;
		}
		...
		if(inOut.getDocStatus().equals(MInOut.DOCSTATUS_Completed)){
			...
			rma.setClientOrg(AD_Client_ID, AD_Org_ID);
			...
			retHead.setClientOrg(AD_Client_ID, AD_Org_ID);
			...
			ret.setClientOrg(inOut.getAD_Client_ID(), inOut.getAD_Org_ID());
			...
			for(MRMALine line : lines){
				...
				inLine.setClientOrg(inOut.getAD_Client_ID(),inOut.getAD_Org_ID());
			...
			for(MInOutLine line : retLines){
				...
				ARCrMemoLine.saveEx();
//				ARCrMemoLine.setClientOrg(line.getAD_Client_ID(), line.getAD_Org_ID());
//				ARCrMemoLine.setC_Invoice_ID(ARCreditMemo.getC_Invoice_ID());
//				ARCrMemoLine.setC_OrderLine_ID(line.getC_OrderLine_ID());
//				ARCrMemoLine.setM_InOutLine_ID(line.getM_InOutLine_ID());
//				ARCrMemoLine.setM_Product_ID(line.getM_Product_ID());
//				ARCrMemoLine.setDescription(line.getDescription());
//				ARCrMemoLine.setQtyEntered(line.getQtyEntered());
//				ARCrMemoLine.setQtyInvoiced(line.getQtyEntered());
//				ARCrMemoLine.setC_UOM_ID(line.getC_UOM_ID());
//				ARCrMemoLine.setPriceEntered(rmaLine.getAmt());
//				ARCrMemoLine.setPriceActual(rmaLine.getAmt());
//				ARCrMemoLine.setPriceList(ordLine.getPriceList());
//				ARCrMemoLine.setLineNetAmt();
//				ARCrMemoLine.setLineTotalAmt(rmaLine.getLineNetAmt());
//				ARCrMemoLine.setC_Tax_ID(rmaLine.getC_Tax_ID());
//				ARCrMemoLine.setPrice(rmaLine.getAmt());
//				ARCrMemoLine.setM_RMALine_ID(rmaLine.getM_RMALine_ID());
//				if(line.getM_AttributeSetInstance_ID() > 0){
//					ARCrMemoLine.setM_AttributeSetInstance_ID(line.getM_AttributeSetInstance_ID());
//				}
//				ARCrMemoLine.setC_Tax_ID(ordLine.getC_Tax_ID());
//				ARCrMemoLine.saveEx();
//				ARCrMemoLine.setTaxAmt();
//				ARCrMemoLine.saveEx();
	...
	public void saveToDecorisWindow(int AR_CreditMemo_ID,int Return_ID,BigDecimal tunai,BigDecimal bank,BigDecimal leasing,BigDecimal hutang,String lease,int Supervidor_ID){
		...
		decPos.setClientOrg(AD_Client_ID, ARCrMemo.getAD_Org_ID());
		...
		if(tunai.compareTo(Env.ZERO) > 0){
			...
			decPos.setPembayaran1(tunai);
			
		}
		if(bank.compareTo(Env.ZERO) > 0){
			
			decPos.setPayType2("BANK");
			decPos.setPembayaran2(bank);
			
		}
		if(leasing.compareTo(Env.ZERO) > 0){
			
			decPos.setPayType3("LEASING");
			decPos.setPembayaran3(leasing);
			
		}
		if(hutang.compareTo(Env.ZERO) > 0){
			
			decPos.setPayType4("HUTANG");
			decPos.setPembayaran4(hutang);
		...
		for (MInvoiceLine line : lines){
			...
			decPOSLine.setM_Locator_ID(returnLine.getM_Locator_ID());
