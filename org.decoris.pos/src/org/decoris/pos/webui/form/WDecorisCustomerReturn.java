 ...
 *
 */
public class WDecorisCustomerReturn extends DecorisCustomerReturn implements IFormController,
	...
	public void onEvent(Event e) throws Exception {
		...
		if(e.getTarget().equals(process)){
			...
			try{
				...
				MDecorisPOS decorispos = new MDecorisPOS(ctx, (Integer)noNota.getValue(), null);
				Timestamp tglNota = decorispos.getDateOrdered();
	...
	public void process(int supervisor_id){
		...
		//	SQLGetOnHand.append(" AND M_Locator_ID = ?");
			...
			if(!IsNegativ){
				...
				SQLUpdate.append("UPDATE C_DecorisPOS ");
				...
				SQLUpdate.append(" AND C_DecorisPOS_ID = "+ pos.getC_DecorisPOS_ID());
...
//		}
		...
		MInvoice invCek = new MInvoice(ctx, C_InvoiceOld_ID, null);
			...
			if(!invCek.isPaid()){
				...
				createAutoARReceipt(false, invCek.getC_Invoice_ID(),ARCreditMemo.getC_Invoice_ID(), CreatedByPOS_ID, C_DecorisPOS_ID, ret.getM_InOut_ID(),dateShip,C_PaymentOld_ID,supervisor_id);
				
				
			}else if(invCek.isPaid()){
				
					
					createAutoARReceipt(true,invCek.getC_Invoice_ID(),ARCreditMemo.getC_Invoice_ID(), CreatedByPOS_ID, C_DecorisPOS_ID, ret.getM_InOut_ID(),dateShip,C_PaymentOld_ID,supervisor_id);
			...
			panel.addActionListener(Events.ON_CLICK, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
					if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
						...
						saveToDecorisWindow1(AR_CreditMemo_ID,Return_ID,nilaiBayarTunai,nilaiBayarBank,nilaiBayarLeasing,nilaiBayarHutang,lease.toUpperCase(),Supervisor_ID);
						FDialog.info(windowNo, null, "", "Process Return Berhasil", "Info");
						cetakNota.setEnabled(true);			
...
public void saveToDecorisWindow1(int AR_CreditMemo_ID,int Return_ID,BigDecimal tunai,BigDecimal bank,BigDecimal leasing,BigDecimal hutang,String lease,int Supervidor_ID){
