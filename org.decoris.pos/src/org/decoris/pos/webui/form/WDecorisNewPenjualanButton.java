...
import org.decoris.model.X_C_DecorisLineTemp;
import org.decoris.model.X_C_DecorisTemp;
import org.decoris.model.X_C_Decoris_PreOrder;
import org.decoris.model.X_C_Decoris_PreSales;
import org.decoris.process.DecorisProcessPOS;
 ...
 *
 */
public class WDecorisNewPenjualanButton extends DecorisNewPenjualanButton implements IFormController,
	...
	public WDecorisNewPenjualanButton() {
	...
	private void initUI() {
		...
		row2 = rows2.newRow();
		row2.appendCellChild(NoPreOrderLabel.rightAlign(), 1);
		row2.appendCellChild(noPreOrder.getComponent(), 2);
		...
		row2 = rows2.newRow();
		row2.appendCellChild(IMEILabel.rightAlign(), 1);
		row2.appendCellChild(IMEIList, 2);
		...
		row3.appendCellChild(tanggalSearch, 2);
		...
		row3.appendCellChild(addProduct, 2);
		addProduct.setHflex("true");
		...
		row3 = rows3.newRow();
		row3.appendCellChild(PickUpLabel.rightAlign(), 1);
		Hbox pickupHbox = new Hbox();
		row3.appendCellChild(pickupHbox, 2);
		...
		TotalHeaderTextBox.setStyle("font-size:20px;font-weight:bold;text-align:right;border: 1px solid #000000;");
	...
	private boolean dyinit() {
		...
		int org_id = DB.getSQLValueEx(ctx.toString(), sqlOrg, new Object[] {AD_Client_ID, CreatedByPOS_ID });
		...
		MLookup lookupPreSales = MLookupFactory.get(ctx, form.getWindowNo(), 0,1002329, DisplayType.Search);
		...
		// Initial value presales lookup
		MLookup lookupPreOrder = MLookupFactory.get(ctx, form.getWindowNo(), 0,1007664, DisplayType.Search);
		noPreOrder = new WSearchEditor("C_Decoris_PreOrder_ID", true, false,true, lookupPreOrder);
...
//		ProductSearch.setMandatory(true);
		...
		if (Org_ID == null) {
			FDialog.info(windowNo, null, "","Cabang tidak di ketahui, Silakan hub. Admin", "Info");
			IsOK = false;
			return IsOK;
		}
	...
	public void valueChange(ValueChangeEvent evt) {
		try {
			...
			if (value == null) {
				...
				if (name.equals("C_Decoris_PreSales_ID")||name.equals("C_Decoris_PreOrder_ID")) {	
					salesTable.clear();
			...
			if (name.equals("C_BPartner_ID")) {
				...
				if (sPriceList != null && sPriceList != "" && Lokasi.getText() != null && Lokasi.getText() != "") {
...
//
//				if (productID != null) {
//					Integer M_LocatorPOS_ID = getIDFromComboBox(Lokasi,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
...
//					int M_LocatorProd_ID = DB.getSQLValueEx(ctx.toString(),sqlLocator, new Object[] { AD_Client_ID, productID,M_LocatorPOS_ID });
//					M_Pricelist_ID = getIDFromComboBox(DaftarHargaSearch,MPriceList.Table_Name, MPriceList.COLUMNNAME_Name);
...
//												FDialog.info(windowNo, null, "","Tambah Produk Dibatalkan");
//												w.dispose();
//											} else if (event.getTarget() == panel.getButton(ConfirmPanel.A_OK)) {
//												Integer M_LocatorPOSin_ID = getIDFromComboBox(Lokasi,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
//												Vector<Vector<Object>> data = getProductIMEIData(productID,M_Pricelist_ID, bpartnerId, M_LocatorPOSin_ID, 0,salesTable,Qty.getValue());
...
//												//modelP.addTableModelListener();
//												salesTable.setData(modelP, columnNames);
//												configureMiniTable(salesTable);
//
//												ArrayList<KeyNamePair> list = loadImei(productID,M_LocatorPOSin_ID);
//												IMEIList.removeAllItems();
...
//			} 
				...
				int C_Decoris_PreSales_ID = ((Integer) value).intValue();

				if (C_Decoris_PreSales_ID > 0) {

					X_C_Decoris_PreSales preSales = new X_C_Decoris_PreSales(ctx, C_Decoris_PreSales_ID, null);
					
					if(preSales.getStatus().equals(X_C_Decoris_PreSales.STATUS_Closed)||preSales.getStatus().equals(X_C_Decoris_PreSales.STATUS_Completed)){
						FDialog.info(windowNo,null,"","Tidak dapat diprocess,Status Presales yang anda inputkan sudah Complete atau Closed","Info");
						return;
					}
					...
					if (preSales.getM_Warehouse_ID() > 0) {
						...
						ArrayList<KeyNamePair> listLoc = loadLocator(preSales.getM_Warehouse_ID(), preSales.getAD_Org_ID());
						Lokasi.removeAllItems();
						for (KeyNamePair priceList : listLoc)
							Lokasi.appendItem(priceList.getName());
					...
					TokoSearch.setValue(preSales.getAD_Client_ID());
...
//					}
					...
					if(preSales.getStatus().equals("IP")){
					...
					TotalHeaderTextBox.setText(format.format(preSales.getTotalBelanja()));
				...
				int C_Decoris_PreOrder_ID = ((Integer) value).intValue();

				if (C_Decoris_PreOrder_ID > 0) {

					X_C_Decoris_PreOrder preOrder = new X_C_Decoris_PreOrder(ctx, C_Decoris_PreOrder_ID, null);
					
					boolean isPass = IsPassStatus(C_Decoris_PreOrder_ID);
					
					if(!isPass){
						FDialog.info(windowNo,null,"","Tidak dapat diproses,Status PreOrder yang anda inputkan Belum di Approve","Info");
						return;
					}
					
					if (preOrder != null) {

						Vector<Vector<Object>> data = getPreOrderData(C_Decoris_PreOrder_ID);
						Vector<String> columnNames = getOISColumnNamesExistingPresales();
					...
					if (preOrder.getM_PriceList_ID() > 0) {
						MPriceList PL = new MPriceList(ctx,preOrder.getM_PriceList_ID(), null);
						ArrayList<KeyNamePair> listPL = new ArrayList<KeyNamePair>();
						listPL.add(new KeyNamePair(PL.getM_PriceList_ID(), PL.getName()));
						DaftarHargaSearch.removeAllItems();
						for (KeyNamePair priceL : listPL){
					...
					if (preOrder.get_ValueAsInt("M_Warehouse_ID") > 0) {
						MWarehouse WH = new MWarehouse(ctx,preOrder.get_ValueAsInt("M_Warehouse_ID"), null);
						ArrayList<KeyNamePair> listWH = new ArrayList<KeyNamePair>();
						listWH.add(new KeyNamePair(WH.getM_Warehouse_ID(), WH.getName()));
						GudangSearch.removeAllItems();
						for (KeyNamePair imei : listWH)
					...
					TokoSearch.setValue(preOrder.getAD_Client_ID());
					IMEIList.setEnabled(false);
					noPresales.setValue(preOrder.getC_Decoris_PreSales_ID());
					salesSearch.setValue(preOrder.get_ValueAsInt("SalesRep_ID"));
					salesSearch.setReadWrite(false);
					orgSearch.setValue(preOrder.getAD_Org_ID());
					...
					addProduct.setEnabled(false);
					tanggalSearch.setValue(preOrder.getDateOrdered());
					tanggalSearch.setReadonly(true);
					PickUp.setChecked(true);
					PickUp.setEnabled(false);
					PelangganSearch.setValue(preOrder.getC_BPartner_ID());
					...
					paymentTunai.setValue(preOrder.getdpamt());
					paymentLeasing.setValue(preOrder.gettotprodprice().subtract(preOrder.getdpamt()));
					leasingProviderSearch.setValue("Spektra");
					keteranganTextBox.setValue("Cicilan = " + preOrder.getangsuran()+" " +"\n" + "Tenor = " + preOrder.gettop()+"X");
					

//					if(preOrder.getStatus().equals("IP")){
//						
//						paymentTunai.setReadonly(false);
//						paymentBank.setReadonly(false);
//						paymentLeasing.setReadonly(false);
...
//					BigDecimal totalBayarAll = tunai.add(bank).add(leasing).add(hutang);
//
//					totalBayarTextBox.setText(format.format(preOrder.getTotalBayar()));
//
//					BigDecimal kembali = totalBayar.subtract(preOrder.getTotalBelanja());
//					kembaliTextBox.setText(format.format(kembali));
//					diskonTextBox.setText(format.format(preOrder.getTotalDiskon()));
					totalBelanjaTextBox.setText(format.format(preOrder.gettotprodprice()));
					TotalHeaderTextBox.setText(format.format(preOrder.gettotprodprice()));
					totalBrutoTextBox.setText(format.format(preOrder.gettotprodprice()));

//					totalBayarTunaiTextBox.setText(format.format(preOrder.getTotalBayar()));
//					totalBayarTextBox.setText(format.format(totalBayarAll));
//					kembalianTextBox.setText(format.format(preOrder.get_Value("TotalKembalian")));
//					kembaliTextBox.setText(format.format(preOrder.get_Value("TotalKembalian")));
//					DPPTextBox.setText(format.format(preOrder.getdpp()));
//					PPNTextBox.setText(format.format(preOrder.getTaxAmt()));
//
//					calculateDPPExistingPresales();
				}
	...
	public void onEvent(Event e) throws Exception {
		...
		if (e.getTarget().equals(IMEIList)) {
			...
			//delete();
			//getProductData(M_Product_ID, M_PriceList_ID, C_BPartner_ID, M_Locator_ID, M_AttributeSetInstance_ID, MiniTable);
			int warehouse_ID = getIDFromComboBox(GudangSearch,MWarehouse.Table_Name, MWarehouse.COLUMNNAME_Name);

			addProduct(M_Pricelist_ID, (int)PelangganSearch.getValue(), warehouse_ID,(int)orgSearch.getValue());
		}else if (e.getTarget().equals(addProduct)) {
			...
			Integer PriceList_ID = getIDFromComboBox(DaftarHargaSearch,MPriceList.Table_Name, MPriceList.COLUMNNAME_Name);
		...
		} else if (e.getTarget().equals(paymentTunai)|| e.getTarget().equals(paymentBank)|| e.getTarget().equals(paymentLeasing)|| e.getTarget().equals(paymentHutang)) {
	...
	public void calculate() {
		...
		totalBrutoTextBox.setText(format.format(nilaiGrandTotal.add(totalDiskon)));
	...
	public void newOrder() {
		...
		// Initial value Warehouse
//		ArrayList<KeyNamePair> listWH = loadWarehouse((int) orgSearch.getValue());
//		GudangSearch.removeAllItems();
//		for (KeyNamePair priceList : listWH) {
//			GudangSearch.appendItem(priceList.getName());
//		}
		...
		panel.addActionListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
					...
					ResultSet rs = null;
					try {
						pstmt = DB.prepareStatement(SQLMandatory.toString(), null);

						rs = pstmt.executeQuery();
						while (rs.next()) {

							if(rs.getString(1).toUpperCase() == birthday.toUpperCase() && rs.getString(2).toUpperCase() == "Y"){
								mndtry_bday = true;
							}else if(rs.getString(1).toUpperCase() == Addres2.toUpperCase()&& rs.getString(2).toUpperCase() == "Y"){
								mndtry_addrss = true;
							}else if(rs.getString(1).toUpperCase() == KTPNo.toUpperCase()&& rs.getString(2).toUpperCase() == "Y"){
								mndtry_ktp = true;
							}else if(rs.getString(1).toUpperCase() == zipcode.toUpperCase()&& rs.getString(2).toUpperCase() == "Y"){
								mndtry_zip = true;
							}else if(rs.getString(1).toUpperCase() == phone2.toUpperCase()&& rs.getString(2).toUpperCase() == "Y"){
								mndtry_phone2 = true;
							}else if(rs.getString(1).toUpperCase() == email.toUpperCase()&& rs.getString(2).toUpperCase() == "Y"){
								mndtry_email = true;
							}
					...
					if (BPIDCardNumber.getValue().toString().length() != 16 && BPName.getValue().isEmpty()) {
...
//					}
	...
	private void process() {
		...
		String tipeBayar1 = payruleBoxTunai.getValue();
		String tipeBayar2 = payruleBoxBank.getValue();
		String tipeBayar3 = payruleBoxLeasing.getValue();
		String tipeBayar4 = payruleBoxHutang.getValue();
		System.out.println(tipeBayar1);
		System.out.println(tipeBayar2);
		System.out.println(tipeBayar3);
		System.out.println(tipeBayar4);
		...
		if (AD_Language.toUpperCase().equals("EN_US")) {
			...
			String ddisc = diskonTextBox.getText().replaceAll("\\.", "").replaceAll(",", ".");
		...
		msg = checkBeforeProcess(AD_Org_ID, M_Pricelist_ID, C_BPartner_ID,
				...
				C_Currency_ID, C_BankAccount_ID, payruleBoxTunai.getValue(),
		...
		if (leasingProviderSearch.getValue() != null) {
			...
			getMinimumLeasing.append(" WHERE value = 'Min_Pembiayaan_Spektra' ");
			
			BigDecimal minimumLeasing = DB.getSQLValueBD(null, getMinimumLeasing.toString());
			
			if(payLeasing.compareTo(minimumLeasing)< 0){
				
				FDialog.warn(form.getWindowNo(), null, "Nilai Pembayaran Leasing Kurang dari Nilai Minimum", "","Peringatan");
				return;
			}
		...
		if(payment1==Env.ZERO && payment2==Env.ZERO && payment3==Env.ZERO && payment4==Env.ZERO ){
			
			FDialog.warn(form.getWindowNo(), null, "", "Silahkan Entri Pembayaran Terlebih Dahulu", "Peringatan");
			return;
		...
		X_C_DecorisTemp decPos_temp = new X_C_DecorisTemp(ctx, 0, null);
		...
		decPos_temp.setClientOrg(Env.getAD_Client_ID(ctx),(int) orgSearch.getValue());
		...
		if (payment3.compareTo(Env.ZERO) > 0) {
			...
			if (leasingProviderSearch.getValue().toString().toUpperCase().equals("SPEKTRA")) {
				decPos_temp.set_ValueOfColumnReturningBoolean("IsSpektra", true);
		...
		if (payment2.compareTo(Env.ZERO) > 0) {
			...
			decPos_temp.setC_BankAccount_ID((Integer) bankAccountSearch.getValue());
		}
		if (payment3.compareTo(Env.ZERO) > 0) {
		...
		for (int i = 0; i < miniTable.getRowCount(); i++) {
			...
			X_C_DecorisLineTemp decPOSLine_temp = new X_C_DecorisLineTemp(ctx, 0, decPos_temp.get_TrxName());

			String sqlLine = "SELECT COALESCE(MAX(Line),0)+10 FROM C_DecorisLineTemp WHERE C_DecorisTemp_ID =?";
			int ii = DB.getSQLValue(decPos_temp.get_TrxName(), sqlLine,decPos_temp.getC_DecorisTemp_ID());

			// DecorisLine
			decPOSLine_temp.setClientOrg(decPos_temp);
			decPOSLine_temp.setC_DecorisTemp_ID(decPos_temp.getC_DecorisTemp_ID());
			...
			if (M_AttributeSetInstance_ID > 0) {
				decPOSLine_temp.setM_AttributeSetInstance_ID(M_AttributeSetInstance_ID);
			}
		...
		SQLGetProcess.append(" WHERE Value = 'DecorisPosRollback'");
		...
		list.add(new ProcessInfoParameter("C_DecorisTemp_ID", decPos_temp.getC_DecorisTemp_ID(), null, null,null));
		...
		boolean ada = new DecorisProcessPOS().startProcess(Env.getCtx(), pi, Trx.get(trxName, true));

		System.out.println(ada);
		
		X_C_DecorisTemp decPos_temp2nd = new X_C_DecorisTemp(decPos_temp.getCtx(), decPos_temp.getC_DecorisTemp_ID(), null);
			...
			if (M_InOut_ID > 0) {
				...
				if (DocStatus.equals(MInOut.DOCSTATUS_Drafted)) {
					...
					for (int i = 0; i < miniTable.getRowCount(); i++) {
						...
						BigDecimal POSqQty = ((BigDecimal) miniTable.getValueAt(i, 6)).setScale(2);
		...
		int C_DecorisPOS_ID = decPos_temp2nd.get_ValueAsInt("C_DecorisPOS_ID");
		MDecorisPOS decPos = new MDecorisPOS(decPos_temp2nd.getCtx(), C_DecorisPOS_ID, decPos_temp2nd.get_TrxName());
		...
		pSales.setStatus(X_C_Decoris_PreSales.STATUS_Completed);
		pSales.setC_DecorisPOS_ID(decPos.getC_DecorisPOS_ID());
		pSales.setC_Order_ID(decPos.getC_Order_ID());		
		pSales.saveEx();
		...
		if (isPick) {
			infoGeneratedDocument(decPos_temp2nd.getC_Order_ID(), form.getWindowNo());
		}
		...
		if (!isPick) {
				...
				bedaAlamatCheck.addEventListener(Events.ON_CHECK,
						new EventListener<Event>() {
							...
							public void onEvent(Event ev) throws Exception {
								...
								boolean isDiffAlamat = bedaAlamatCheck.isChecked();
							...
							public void onEvent(Event event) throws Exception {
								if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
									...
									if (tglKirim == null) {
										...
										FDialog.info(windowNo,null,"","Tanggal Kirim Belum Ditentukan","Info");
										return;
									...
									infoGeneratedDocument(ino.getC_Order_ID(),form.getWindowNo());
			...
			decPos_temp2nd.delete(true, decPos_temp2nd.get_TrxName());
...
//			int M_LocatorPOS_ID = getIDFromComboBox(Lokasi,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
//
//			MLocator loc = new MLocator(ctx, M_LocatorPOS_ID, null);
//
...
//		}
			...
			if(countSelected > 1){
				...
				if(isSelect){
					if (imeiProd.equalsIgnoreCase(imeiTab)) {
						...
						int M_Loc_ID = getIDFromComboBox(Lokasi,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
						MLocator loc = new MLocator(ctx, M_Loc_ID, null);
						rowidx = i;
						
						KeyNamePair LKP = (KeyNamePair)salesTable.getValueAt(i, 7);
		...
		if (rowidx > 0) {
			KeyNamePair product = (KeyNamePair) salesTable.getValueAt(rowidx, 1);
	...
	private void priceList() {
		...
		Integer M_PL_ID = getIDFromComboBox(DaftarHargaSearch,MPriceList.Table_Name, MPriceList.COLUMNNAME_Name);
		...
		M_Pricelist_ID = DB.getSQLValueEx(ctx.toString(),sqlPriceList.toString(), new Object[] { AD_Client_ID,AD_Client_ID, CreatedByPOS_ID, M_PL_ID });
	...
	public String validation (){
		...
		if(ptunai.compareTo(Env.ZERO)<0||pbank.compareTo(Env.ZERO)<0||pleasing.compareTo(Env.ZERO)<0||phutang.compareTo(Env.ZERO)<0){
			msg = msg + "\n" +"Metode Pembayaran Tidak Boleh Kurang Dari Nol";

		}
		
		if(pbank.compareTo(Env.ZERO)>0 || pleasing.compareTo(Env.ZERO)>0 ||phutang.compareTo(Env.ZERO)>0){
		...
		}else if(ptunai.compareTo(Env.ZERO)>0 && pbank.compareTo(Env.ZERO)==0 && pleasing.compareTo(Env.ZERO)==0 && phutang.compareTo(Env.ZERO)==0){
	...
	private void addProduct(int M_PriceList_ID,int C_BPartner_ID, int M_WareHouse_ID, int AD_Org_ID){
		

		
		Grid inputProduct = GridFactory.newGridLayout();
		Panel paraPanel = new Panel();
		Rows rows = null;
		Row row = null;

		
		String title = "Input Product";
		Label LabelProduct = new Label("Produk");
		Label LabelIMEI = new Label("IMEI");
		Label LabelQty = new Label("Qty");
		Label LabelPriceList = new Label("Price List");
		Label LabelPrice = new Label("Price");
		Label LabelLocator = new Label("Lokasi"); 

		MLookup lookupProduct = MLookupFactory.get(ctx, form.getWindowNo(), 0,1402, DisplayType.Search);

		final WSearchEditor productSearch = new WSearchEditor("M_Product_ID", true, false, true,lookupProduct);
		final Combobox IMEITB = new Combobox();
		Textbox QtyTB = new Textbox();
		Decimalbox PriceListDB = new Decimalbox();
		Decimalbox PriceDB = new Decimalbox();
		final Combobox LocatorCB = new Combobox();
		...
		mainbBorder.setHeight("100%");
		String grid = "border: 1px solid #C0C0C0; border-radius:5px; vertical-align:bottom;overflow:auto;";
		paraPanel.appendChild(inputProduct);
		...
		inputProduct.setWidth("100%");
		inputProduct.setStyle("Height:100%;");

		rows = inputProduct.newRows();

		// Product
		row = rows.newRow();
		row.appendCellChild(LabelProduct.rightAlign());
		row.appendCellChild(productSearch.getComponent(),3);
		productSearch.addValueChangeListener(new ValueChangeListener() {
			
			@Override
			public void valueChange(ValueChangeEvent evt) {
				int locator_ID = getIDFromComboBox(LocatorCB,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
				Integer productID = (Integer)evt.getNewValue();

				ArrayList<KeyNamePair> list = loadImei(productID,locator_ID);
				IMEITB.removeAllItems();
				for (KeyNamePair imei : list)
					IMEITB.appendItem(imei.getName());
				IMEITB.setSelectedIndex(0);
		...
		// IMEI
		row = rows.newRow();
		row.appendCellChild(LabelLocator.rightAlign());
		row.appendCellChild(LocatorCB, 2);
		LocatorCB.setHflex("true");
		LocatorCB.setReadonly(false);
			
		// Intial value locator
		ArrayList<KeyNamePair> listLoc = loadLocator(M_WareHouse_ID, AD_Org_ID);
		LocatorCB.removeAllItems();
		for (KeyNamePair priceList : listLoc) {
			LocatorCB.appendItem(priceList.getName());
		}
		...
		// IMEI
		row = rows.newRow();
		row.appendCellChild(LabelIMEI.rightAlign());
		row.appendCellChild(IMEITB, 2);
		IMEITB.setHflex("true");
		
		// IMEI
		row = rows.newRow();
		row.appendCellChild(LabelPriceList.rightAlign());
		row.appendCellChild(PriceListDB, 2);
		PriceListDB.setHflex("true");
		PriceListDB.setReadonly(true);
		PriceListDB.setValue("0.00");
		PriceListDB.setStyle("text-align:right;");
		
		
	
		// IMEI
		row = rows.newRow();
		row.appendCellChild(LabelPrice.rightAlign());
		row.appendCellChild(PriceDB, 2);
		PriceDB.setHflex("true");
		PriceDB.setValue("0.00");
		PriceDB.setStyle("text-align:right;");

		// Qty
		row = rows.newRow();
		row.appendCellChild(LabelQty.rightAlign());
		row.appendCellChild(QtyTB,2);
		QtyTB.setHflex("true");
		...
		panel.addActionListener(Events.ON_CLICK, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				if (event.getTarget() == panel.getButton(ConfirmPanel.A_CANCEL)) {
					...
					int product_ID = (int)productSearch.getValue(); 
					int pricelist_ID = getIDFromComboBox(DaftarHargaSearch,MPriceList.Table_Name, MPriceList.COLUMNNAME_Name);
					int bp_ID = (int) PelangganSearch.getValue();
					int locator_ID = getIDFromComboBox(LocatorCB,MLocator.Table_Name, MLocator.COLUMNNAME_Value);
					int imei_ID = getIDFromComboBox(IMEITB,MAttributeSetInstance.Table_Name, MAttributeSetInstance.COLUMNNAME_SerNo);

					
					
					
					
					Vector<Vector<Object>> data = getProductData(product_ID, pricelist_ID, bp_ID, locator_ID, imei_ID, salesTable);
					Vector<String> columnNames = getOISColumnNames();
