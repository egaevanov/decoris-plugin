...
public class WPosPaymentWindow extends PosPaymentWindow implements IFormController,EventListener<Event>, WTableModelListener, ValueChangeListener{
	...
	private void process(){
		...
		decPos.setAD_Org_ID(m_AD_org_ID);
		decPos.setC_BPartner_ID(m_C_BPartner_ID);
		decPos.setDateOrdered(tglTrx);
		decPos.setDateTrx(tglTrx);
		...
		X_C_DecorisPOSLine decPOSLine = new X_C_DecorisPOSLine(ctx, 0, decPos.get_TrxName());
		
		String sqlLine = "SELECT COALESCE(MAX(Line),0)+10 FROM C_DecorisPOSLine WHERE AD_Client_ID = ? AND C_DecorisPOS_ID =?";
		int ii = DB.getSQLValue (decPos.get_TrxName(), sqlLine, new Object[]{AD_Client_ID,decPos.getC_DecorisPOS_ID()});
		...
		sqlDecorisPOS.append("SELECT PayType1,PayType2,Pembayaran1,Pembayaran2 ");
		sqlDecorisPOS.append("FROM C_DecorisPOS ");
		sqlDecorisPOS.append("WHERE AD_Client_ID =? ");
		sqlDecorisPOS.append("AND C_DecorisPOS_ID =? ");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = DB.prepareStatement(sqlDecorisPOS.toString(), null);
			pstmt.setInt(1, AD_Client_ID);
			pstmt.setInt(2, decPos.getC_DecorisPOS_ID());
			...
			while (rs.next()) {
				...
				inv.setClientOrg(AD_Client_ID,decPos.getAD_Org_ID());
				...
				if (inv.getDocStatus().equals(MInvoice.DOCSTATUS_Completed)){
					...
					for (String key : mapPospay.keySet() ){
						...
						pay.setClientOrg(AD_Client_ID,decPos.getAD_Org_ID());
			...
			log.log(Level.SEVERE, sqlDecorisPOS.toString(), ex);
		} finally {
			DB.close(rs, pstmt);
			rs = null;
			pstmt = null;
		...
		noNotaTB.setText(decPos.getDocumentNo().toString());
